    ...
    if uploaded_file is not None: 
        ...
        if has_missing_values or has_duplicates: 
            ...
            if has_missing_values: 
                ...
                for column in missing_values[missing_values>0].index: 
                    ...
                    if st.button(f"Fill missing values in '{column}' with median"):
                        ...
                        df[column] = imputer.fit_transform(df[[column]]).astype(original_dtype)
            ...
            if has_duplicates:
                ...
                st.write(f"Number of duplicated rows : {duplicate_count}")
                ...
                if st.button("Remove Duplicate Rows : "):
                    ...
                    st.success("Duplicate rows are removed successfully")
...
def select_target_column(df):
    if df is not None: 
        ...
        if target_column != "": 
            ...
            st.write(x)
            st.write('Target Features(y)')
            st.write(y)
            
def feature_selection_options(): 
    feature_selection_method = st.selectbox("Select Feature Selection Method", ["None", "Correlation Coefficient", "Chi-Square Test", "Anova", "Mutual Information", "Variance Threshold"], 
    index=0)
...
def save_selected_features():  
    ...
    if 'selected_features' in st.session_state and 'y' in st.session_state: 
        ...
        if not selected_features: 
            st.error("No Features Selected! Adjust your threshold.")
            return
        ...
        st.download_button(
            ...
            file_name='selected_features_target.csv', 
            mime='text/csv'
        )
...
def chi_square_selection(): 
    if 'x' in st.session_state and 'y' in st.session_state: 
        ...
        selected_featurees = x.columns[selected_indices].tolist()
        
        # display selected features
        st.write("Selected Features : ")
        st.write(selected_featurees)
        
        # update session state 
        st.session_state['selected_features'] = selected_featurees
...
def anova_selection(): 
    if 'x' in st.session_state and 'y' in st.session_state: 
        ...
        # update session state 
        st.session_state['selected_features'] = selected_featurees
        st.success("Features selected based on Anova Technique.")
...
def mutual_info_selection(): 
    if 'x' in st.session_state and 'y' in st.session_state: 
        ...
        # update session state 
        st.session_state['selected_features'] = selected_featurees
        st.success("Features selected based on Mutual Information Technique.")
